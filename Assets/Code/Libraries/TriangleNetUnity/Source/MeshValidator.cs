...
namespace TriangleNet
{
    ...
    public static class MeshValidator
    {
        ...
        {
            return IsDelaunay(triangleNetMesh, false);
        }
        ...
        {
            return IsDelaunay(triangleNetMesh, true);
        }
        ...
        private static bool IsDelaunay(TriangleNetMesh triangleNetMesh, bool constrained)
        {
            ...
            var inf1 = triangleNetMesh.infvertex1;
            var inf2 = triangleNetMesh.infvertex2;
            var inf3 = triangleNetMesh.infvertex3;
            ...
            foreach (var tri in triangleNetMesh.triangles)
            {
                ...
                for (loop.orient = 0; loop.orient < 3; loop.orient++)
                {
                    ...
                    shouldbedelaunay = (loop.tri.id < oppotri.tri.id) &&
                           !Otri.IsDead(oppotri.tri) && (oppotri.tri.id != TriangleNetMesh.DUMMY) &&
                    ...
                    if (constrained && triangleNetMesh.checksegments && shouldbedelaunay)
                    {
                        // If a subsegment separates the triangles, then the edge is
                        // constrained, so no local Delaunay test should be done.
                        loop.Pivot(ref opposubseg);

                        if (opposubseg.seg.hash != TriangleNetMesh.DUMMY)
                        {
