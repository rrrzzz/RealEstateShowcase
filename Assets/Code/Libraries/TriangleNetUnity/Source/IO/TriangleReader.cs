...
namespace TriangleNet.IO
{
    ...
    public class TriangleReader
    {
        ...
        private void ReadVertex(List<Vertex> data, int index, string[] line, int attributes, int marks)
        {
            ...
            {
#if USE_ATTRIBS
                var attribs = new float[attributes];
                ...
                for (int j = 0; j < attributes; j++)
                {
                    if (line.Length > 3 + j)
                    {
                        attribs[j] = float.Parse(line[3 + j], nfi);
                    }
                }
            ...
            using (var reader = new StreamReader(polyfilename))
            {
                ...
                if (holes > 0)
                {
                    for (int i = 0; i < holes; i++)
                    {
                        ...
                        data.Holes.Add(new Point(float.Parse(line[1], nfi),
                ...
                if (TryReadLine(reader, out line))
                {
                    ...
                    if (regions > 0)
                    {
                        for (int i = 0; i < regions; i++)
                        {
                            ...
                            if (line.Length > 4)
                            {
                                float.TryParse(line[4], NumberStyles.Number, nfi, out area);
                            }
                            ...
                            data.Regions.Add(new RegionPointer(
                                float.Parse(line[1], nfi), // Region x
                                float.Parse(line[2], nfi), // Region y
                                id, area));
                        }
                    }
                }
        ...
        /// <param name="data"></param>
        private float[] ReadAreaFile(string areafilename, int intriangles)
        {
            float[] data = null;
