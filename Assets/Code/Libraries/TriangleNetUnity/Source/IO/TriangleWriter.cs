...
namespace TriangleNet.IO
{
    ...
    public class TriangleWriter
    {
        ...
        public void Write(TriangleNetMesh triangleNetMesh, string filename)
        {
            WritePoly(triangleNetMesh, Path.ChangeExtension(filename, ".poly"));
            WriteElements(triangleNetMesh, Path.ChangeExtension(filename, ".ele"));
        }
        ...
        /// <param name="triangleNetMesh"></param>
        /// <param name="filename"></param>
        public void WritePoly(TriangleNetMesh triangleNetMesh, string filename)
        {
            WritePoly(triangleNetMesh, filename, true);
        }

        /// <summary>
        /// Write the segments and holes to a .poly file.
        /// </summary>
        /// <param name="triangleNetMesh">Data source.</param>
        /// <param name="filename">File name.</param>
        /// <param name="writeNodes">Write nodes into this file.</param>
        /// <remarks>If the nodes should not be written into this file, 
        /// make sure a .node file was written before, so that the nodes 
        /// are numbered right.</remarks>
        public void WritePoly(TriangleNetMesh triangleNetMesh, string filename, bool writeNodes)
        {
            Osub subseg = default(Osub);
            Vertex pt1, pt2;

            bool useBoundaryMarkers = triangleNetMesh.behavior.UseBoundaryMarkers;
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                else
                {
                    ...
                    // and number of boundary markers (zero or one).
                    writer.WriteLine("0 {0} {1} {2}", triangleNetMesh.mesh_dim, triangleNetMesh.nextras,
                        useBoundaryMarkers ? "1" : "0");
                }

                // Number of segments, number of boundary markers (zero or one).
                writer.WriteLine("{0} {1}", triangleNetMesh.subsegs.Count,
                    useBoundaryMarkers ? "1" : "0");
                ...
                // Holes
                j = 0;
                writer.WriteLine("{0}", triangleNetMesh.holes.Count);
                foreach (var hole in triangleNetMesh.holes)
                {
                    writer.WriteLine("{0} {1} {2}", j++, hole.X.ToString(nfi), hole.Y.ToString(nfi));
                }

                // Regions
                if (triangleNetMesh.regions.Count > 0)
                {
                    j = 0;
                    writer.WriteLine("{0}", triangleNetMesh.regions.Count);
                    foreach (var region in triangleNetMesh.regions)
                    {
                        writer.WriteLine("{0} {1} {2} {3}", j, region.point.X.ToString(nfi),
                            region.point.Y.ToString(nfi), region.id);
        ...
        /// <param name="triangleNetMesh"></param>
        /// <param name="filename"></param>
        public void WriteEdges(TriangleNetMesh triangleNetMesh, string filename)
        {
            ...
            using (var writer = new StreamWriter(filename))
            {
                ...
                writer.WriteLine("{0} {1}", triangleNetMesh.NumberOfEdges, behavior.UseBoundaryMarkers ? "1" : "0");
                ...
                foreach (var item in triangleNetMesh.triangles)
                {
                    ...
                    {
                        tri.Sym(ref trisym);
                        if ((tri.tri.id < trisym.tri.id) || (trisym.tri.id == TriangleNetMesh.DUMMY))
                        {
                            p1 = tri.Org();
                            p2 = tri.Dest();
                            ...
                            if (behavior.UseBoundaryMarkers)
                            {
                                ...
                                if (behavior.useSegments)
                                {
                                    ...
                                    if (checkmark.seg.hash == TriangleNetMesh.DUMMY)
                                    {
                                ...
                                {
                                    writer.WriteLine("{0} {1} {2} {3}", index, p1.id, p2.id,
                                            trisym.tri.id == TriangleNetMesh.DUMMY ? "1" : "0");
                                }
                            }
                            else
                            {
