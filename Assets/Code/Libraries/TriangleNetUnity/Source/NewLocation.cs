...
namespace TriangleNet
{
    ...
    class NewLocation
    {
        ...
        float[] petalx = new float[20];
        float[] petaly = new float[20];
        float[] petalr = new float[20];
        float[] wedges = new float[500];
        float[] initialConvexPoly = new float[500];

        // Work arrays for smoothing
        float[] points_p = new float[500];
            ...
            ref float xi, ref float eta, bool offcenter, Otri badotri)
        {
            ...
            else
            {
                ...
                //   division by zero.
                denominator = 0.5f / predicates.CounterClockwise(tdest, tapex, torg);
            ...
            if (almostGood == 1)
            {
                ...
                cosMaxAngle = (middleEdgeDist + shortestEdgeDist - longestEdgeDist) / (2 * Mathf.Sqrt(middleEdgeDist) * Mathf.Sqrt(shortestEdgeDist));
                if (cosMaxAngle < 0.0f)
                {
                    // obtuse
                    isObtuse = true;
                }
                else if (Mathf.Abs(cosMaxAngle - 0.0f) <= EPS)
                {
                ...
                if (relocated > 0)
                {
                    ...
                    switch (relocated)
                    {
                        case 1:
                            //printf("Relocate: (%f,%f)\n", torg[0],torg[1]);			
                            _TriangleNetMesh.DeleteVertex(ref delotri);
                            break;
                        case 2:
                            //printf("Relocate: (%f,%f)\n", tdest[0],tdest[1]);			
                            delotri.Lnext();
                ...
                else
                {
                    ...
                    petalRadius = Mathf.Sqrt(shortestEdgeDist) / (2 * Mathf.Sin(behavior.MinAngle * Mathf.PI / 180.0f));
                    ...
                    xPetalCtr_1 = xMidOfShortestEdge + Mathf.Sqrt(petalRadius * petalRadius - (shortestEdgeDist / 4)) * (middleAngleCorner.y -
                        largestAngleCorner.y) / Mathf.Sqrt(shortestEdgeDist);
                    yPetalCtr_1 = yMidOfShortestEdge + Mathf.Sqrt(petalRadius * petalRadius - (shortestEdgeDist / 4)) * (largestAngleCorner.x -
                        middleAngleCorner.x) / Mathf.Sqrt(shortestEdgeDist);

                    xPetalCtr_2 = xMidOfShortestEdge - Mathf.Sqrt(petalRadius * petalRadius - (shortestEdgeDist / 4)) * (middleAngleCorner.y -
                        largestAngleCorner.y) / Mathf.Sqrt(shortestEdgeDist);
                    yPetalCtr_2 = yMidOfShortestEdge - Mathf.Sqrt(petalRadius * petalRadius - (shortestEdgeDist / 4)) * (largestAngleCorner.x -
                        middleAngleCorner.x) / Mathf.Sqrt(shortestEdgeDist);
                    ...
                    if (!neighborNotFound)
                    {
                        ...
                        { // there is at least one intersection point
                            ...
                            if (Mathf.Abs(voronoiOrInter[0] - 1.0f) <= EPS)
                            {
                            ...
                            { // there is no voronoi vertex between intersection point and circumcenter
                                if (IsBadTriangleAngle(largestAngleCorner.x, largestAngleCorner.y, middleAngleCorner.x, middleAngleCorner.y, inter_x, inter_y))
                                {
                                    ...
                                    d = Mathf.Sqrt((inter_x - myCircumcenter.x) * (inter_x - myCircumcenter.x) +
                                    ...
                                    inter_x = inter_x + ax * pertConst * Mathf.Sqrt(shortestEdgeDist);
                                    inter_y = inter_y + ay * pertConst * Mathf.Sqrt(shortestEdgeDist);
            ...
            ref float xi, ref float eta, bool offcenter, Otri badotri)
        {
            ...
            if (almostGood == 1)
            {
                ...
                else
                {
                    ...
                    minangle = Mathf.Acos((middleEdgeDist + longestEdgeDist - shortestEdgeDist) / (2 * Mathf.Sqrt(middleEdgeDist) * Mathf.Sqrt(longestEdgeDist))) * 180.0f / Mathf.PI;
                    ...
                    petalRadius = Mathf.Sqrt(shortestEdgeDist) / (2 * Mathf.Sin(minangle * Mathf.PI / 180.0f));
                    /// compute two possible centers of the petal ///
                    // finding the center
                    // first find the middle point of smallest edge
                    xMidOfShortestEdge = (middleAngleCorner.x + largestAngleCorner.x) / 2.0f;
                    ...
                    dist = Mathf.Sqrt((xPetalCtr - xMidOfShortestEdge) * (xPetalCtr - xMidOfShortestEdge) + (yPetalCtr - yMidOfShortestEdge) * (yPetalCtr - yMidOfShortestEdge));
                    ...
                    alpha = (2.0f * behavior.MaxAngle + minangle - 180.0f) * Mathf.PI / 180.0f;
                    // rotate the vector cw around the petal center			
                    x_1 = petal_bisector_x * Mathf.Cos(alpha) + petal_bisector_y * Mathf.Sin(alpha) + xPetalCtr - xPetalCtr * Mathf.Cos(alpha) - yPetalCtr * Mathf.Sin(alpha);
                    y_1 = -petal_bisector_x * Mathf.Sin(alpha) + petal_bisector_y * Mathf.Cos(alpha) + yPetalCtr + xPetalCtr * Mathf.Sin(alpha) - yPetalCtr * Mathf.Cos(alpha);
                    // rotate the vector ccw around the petal center			
                    x_2 = petal_bisector_x * Mathf.Cos(alpha) - petal_bisector_y * Mathf.Sin(alpha) + xPetalCtr - xPetalCtr * Mathf.Cos(alpha) + yPetalCtr * Mathf.Sin(alpha);
                    y_2 = petal_bisector_x * Mathf.Sin(alpha) + petal_bisector_y * Mathf.Cos(alpha) + yPetalCtr - xPetalCtr * Mathf.Sin(alpha) - yPetalCtr * Mathf.Cos(alpha);
                    ...
                    if (!neighborNotFound_first)
                    {
                        ...
                        { // there is at least one intersection point
                            ...
                            if (Mathf.Abs(voronoiOrInter[0] - 1.0f) <= EPS)
                            {
                                ...
                                if (Mathf.Abs(line_result[0] - 1.0f) <= EPS && line_p[0] > 0.0)
                                {
                                ...
                                lengthConst * ((smallestAngleCorner.x - line_inter_x) *
                                    ...
                                    { // slab intersection point is further away
                                        if (IsBadTriangleAngle(middleAngleCorner.x, middleAngleCorner.y, largestAngleCorner.x, largestAngleCorner.y, line_inter_x, line_inter_y))
                                        {
                                            // apply perturbation
                                            // find the distance between circumcenter and intersection point
                                            d = Mathf.Sqrt((line_inter_x - myCircumcenter.x) * (line_inter_x - myCircumcenter.x) +
                                            ...
                                            ay = ay / d;
                                            // now calculate the new intersection point which is perturbated towards the circumcenter
                                            line_inter_x = line_inter_x + ax * pertConst * Mathf.Sqrt(shortestEdgeDist);
                                            line_inter_y = line_inter_y + ay * pertConst * Mathf.Sqrt(shortestEdgeDist);
                                            if (IsBadTriangleAngle(middleAngleCorner.x, middleAngleCorner.y, largestAngleCorner.x, largestAngleCorner.y, line_inter_x, line_inter_y))
                                            {
                                                // go back to circumcenter
                                                dxFirstSuggestion = dx;
                    ...
                    int whichPoint, ref float[] points)
        {
            ...
            } while (!((Mathf.Abs(returnPoint[0] - third_x) <= EPS) &&
                     (Mathf.Abs(returnPoint[1] - third_y) <= EPS)));
                        ...
                        ref float[] thirdpoint, ref Otri neighotri)
        {
            ...
            for (badotri.orient = 0; badotri.orient < 3; badotri.orient++)
            {
                ...
                if (neighbor.tri.id != TriangleNetMesh.DUMMY)
                {
                    ...
                    else
                    {
                        ...
                        if ((Mathf.Abs(first_x - neighborvertex_1.x) < EPS) &&
                             (Mathf.Abs(first_y - neighborvertex_1.y) < EPS))
                        {
                            firstVertexMatched = 11; // neighbor's 1st vertex is matched to first vertex

                        }
                        else if ((Mathf.Abs(first_x - neighborvertex_2.x) < EPS) &&
                               (Mathf.Abs(first_y - neighborvertex_2.y) < EPS))
                        {
                            firstVertexMatched = 12; // neighbor's 2nd vertex is matched to first vertex

                        }
                        else if ((Mathf.Abs(first_x - neighborvertex_3.x) < EPS) &&
                                   (Mathf.Abs(first_y - neighborvertex_3.y) < EPS))
                        {
                     ...
                     // none of them matched
                        ...
                        if ((Mathf.Abs(second_x - neighborvertex_1.x) < EPS) &&
                            (Mathf.Abs(second_y - neighborvertex_1.y) < EPS))
                        {
                            secondVertexMatched = 21; // neighbor's 1st vertex is matched to second vertex
                        }
                        else if ((Mathf.Abs(second_x - neighborvertex_2.x) < EPS) &&
                           (Mathf.Abs(second_y - neighborvertex_2.y) < EPS))
                        {
                            secondVertexMatched = 22; // neighbor's 2nd vertex is matched to second vertex
                        }
                        else if ((Mathf.Abs(second_x - neighborvertex_3.x) < EPS) &&
                               (Mathf.Abs(second_y - neighborvertex_3.y) < EPS))
                        {
            ...
            float[] points, ref float[] newloc)
        {
            ...
            // minimum angle
            alpha = behavior.MinAngle * Mathf.PI / 180.0f;
            // initialize the constants
            if (behavior.goodAngle == 1.0f)
            {
            ...
            {
                petalcenterconstant = 0.5f / Mathf.Tan(alpha);
                petalradiusconstant = 0.5f / Mathf.Sin(alpha);
            }
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                x01 = x1 - x0;
                y01 = y1 - y0;
                d01 = Mathf.Sqrt(x01 * x01 + y01 * y01);
                ...
                // distance between xmid and petal center	
                dist = Mathf.Sqrt((petalx[i / 2] - xmid) * (petalx[i / 2] - xmid) + (petaly[i / 2] - ymid) * (petaly[i / 2] - ymid));
                // find the unit vector goes from mid point to petal center
                ...
                x_1 = x1 * Mathf.Cos(alpha) - y1 * Mathf.Sin(alpha) + x0 - x0 * Mathf.Cos(alpha) + y0 * Mathf.Sin(alpha);
                y_1 = x1 * Mathf.Sin(alpha) + y1 * Mathf.Cos(alpha) + y0 - x0 * Mathf.Sin(alpha) - y0 * Mathf.Cos(alpha);
                ...
                x_2 = x0 * Mathf.Cos(alpha) + y0 * Mathf.Sin(alpha) + x1 - x1 * Mathf.Cos(alpha) - y1 * Mathf.Sin(alpha);
                y_2 = -x0 * Mathf.Sin(alpha) + y0 * Mathf.Cos(alpha) + y1 + x1 * Mathf.Sin(alpha) - y1 * Mathf.Cos(alpha);
                // add these to wedges list as lines in order	
                ...
                for (j = 1; j < 4; j++)
                {
                    ...
                    x_3 = x3 * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) + y3 * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) + petalx[i / 2] - petalx[i / 2] * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) - petaly[i / 2] * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j);
                    y_3 = -x3 * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) + y3 * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) + petaly[i / 2] + petalx[i / 2] * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) - petaly[i / 2] * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j);
                    // add these to wedges list as lines in order	
                ...
                for (j = 1; j < 4; j++)
                {
                    ...
                    x_4 = x3 * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) - y3 * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) + petalx[i / 2] - petalx[i / 2] * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) + petaly[i / 2] * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j);
                    y_4 = x3 * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) + y3 * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j) + petaly[i / 2] - petalx[i / 2] * Mathf.Sin((Mathf.PI / 3.0f - alpha) * j) - petaly[i / 2] * Mathf.Cos((Mathf.PI / 3.0f - alpha) * j);
        ...
        private bool GetWedgeIntersection(int numpoints, float[] points, ref float[] newloc)
        {
            ...
            beta = behavior.MaxAngle * Mathf.PI / 180.0f;
            ...
            for (i = 0; i < numpoints * 2; i = i + 2)
            {
                ...
                for (j = 1; j < line345; j++)
                {
                    ...
                    x_3 = x3 * Mathf.Cos((alpha / (line345 - 1.0f)) * j) + y3 * Mathf.Sin(((alpha / (line345 - 1.0f)) * j)) + petalx[i / 2] - petalx[i / 2] * Mathf.Cos(((alpha / (line345 - 1.0f)) * j)) - petaly[i / 2] * Mathf.Sin(((alpha / (line345 - 1.0f)) * j));
                    y_3 = -x3 * Mathf.Sin(((alpha / (line345 - 1.0f)) * j)) + y3 * Mathf.Cos(((alpha / (line345 - 1.0f)) * j)) + petaly[i / 2] + petalx[i / 2] * Mathf.Sin(((alpha / (line345 - 1.0f)) * j)) - petaly[i / 2] * Mathf.Cos(((alpha / (line345 - 1.0f)) * j));
                    // add these to wedges list as lines in order	
                ...
                for (j = 1; j < line789; j++)
                {
                    ...
                    x_4 = x3 * Mathf.Cos((alpha / (line789 - 1.0f)) * j) - y3 * Mathf.Sin((alpha / (line789 - 1.0f)) * j) + petalx[i / 2] - petalx[i / 2] * Mathf.Cos((alpha / (line789 - 1.0f)) * j) + petaly[i / 2] * Mathf.Sin((alpha / (line789 - 1.0f)) * j);
                    y_4 = x3 * Mathf.Sin((alpha / (line789 - 1.0f)) * j) + y3 * Mathf.Cos((alpha / (line789 - 1.0f)) * j) + petaly[i / 2] - petalx[i / 2] * Mathf.Sin((alpha / (line789 - 1.0f)) * j) - petaly[i / 2] * Mathf.Cos((alpha / (line789 - 1.0f)) * j);
                    ...
                    for (k = 0; k < 2 * numpoints; k = k + 2)
                    {
                        for (e = 1; e < n; e = e + 1)
                        {
                            ...
                            for (i = 0; i < 2 * numpoints; i = i + 2)
                            {
                                ...
                                {
                                    weight = (1.0f - 0.1f * e * weight) / (float)(numpoints - 1.0);
                        ...
                        float x2, float y2, float x3, float y3)
        {
            ...
            /// calculate cosine of largest angle	///	
            cosAngle = (dist12 + dist23 - dist31) / (2 * Mathf.Sqrt(dist12) * Mathf.Sqrt(dist23));
            ...
            if (Mathf.Acos(cosAngle) < 2 * Mathf.Acos(Mathf.Sqrt(behavior.goodAngle)))
            {
            ...
            float x4, float y4, ref float[] p)
        {
            ...
            if (Mathf.Abs(denom - 0.0f) < EPS && (Mathf.Abs(u_b - 0.0f) < EPS && Mathf.Abs(u_a - 0.0f) < EPS))
            {
                p[0] = 0.0f;
            }
            // if denominator equals to zero, lines are parallel
            else if (Mathf.Abs(denom - 0.0f) < EPS)
            {
                p[0] = 0.0f;
            }
            else
        ...
        private int HalfPlaneIntersection(int numvertices, ref float[] convexPoly, float x1, float y1, float x2, float y2)
        {
            ...
            else
            {
                for (i = 0; i < numpolys; i++)
                {
                    ...
                    z = (Mathf.Abs(min) > Mathf.Abs(max) ? min : max);
        ...
        private int SplitConvexPolygon(int numvertices, float[] convexPoly, float x1, float y1, float x2, float y2, float[][] polys)
        {
            ...
            for (i = 0; i < 2 * numvertices; i = i + 2)
            {
                ...
                if (Mathf.Abs(p[0] - 0.0f) <= compConst)
                {
                ...
                else if (Mathf.Abs(p[0] - 2.0f) <= compConst)
                {
                ...
                else
                {
                    ...
                    if (Mathf.Abs(p[1] - convexPoly[j]) <= compConst && Mathf.Abs(p[2] - convexPoly[j + 1]) <= compConst)
                    {
                        // debug
                        case31++;
                        if (state == 1)
                    ...
                    else if (!(Mathf.Abs(p[1] - convexPoly[i]) <= compConst && Mathf.Abs(p[2] - convexPoly[i + 1]) <= compConst))
                    {
                        // debug
                        case32++;
                        poly1counter++;
        ...
        {
            float z;
            if (Mathf.Atan((y2 - y1) / (x2 - x1)) * 180.0f / Mathf.PI == 90.0f)
            {
                if (Mathf.Abs(x1 - x) <= 0.00000000001f)
                    return 0;
            }
            else
            {
                if (Mathf.Abs(y1 + (((y2 - y1) * (x - x1)) / (x2 - x1)) - y) <= EPS)
                    return 0;
            }
            // third component of the 3 dimensional product
            z = (x2 - x1) * (y - y1) - (y2 - y1) * (x - x1);
            if (Mathf.Abs(z - 0.0f) <= 0.00000000001f)
            {
                return 0;
            }
            else if (z > 0)
            ...
            float x4, float y4, ref float[] p)
        {
            ...
            if (Mathf.Abs(denom - 0.0f) < compConst)
            {
                if (Mathf.Abs(u_b - 0.0f) < compConst && Mathf.Abs(u_a - 0.0f) < compConst)
                {
            ...
            else
            {
                ...
                u_a = u_a / denom;
                // 	    printf("u_b %.20f\n", u_b);
                if (u_b < -compConst || u_b > 1.0f + compConst)
            ...
            float x3, float y3, float r, ref float[] p)
        {
            ...
            else if (i > 0.0 && !(Mathf.Abs(a - 0.0f) < EPS))
            {
                // two intersections
                p[0] = 2.0f;
                // first intersection
                mu = (-b + Mathf.Sqrt(i)) / (2 * a);
                p[1] = x1 + mu * (x2 - x1);
                p[2] = y1 + mu * (y2 - y1);
                // second intersection
                mu = (-b - Mathf.Sqrt(i)) / (2 * a);
        ...
        private bool IsBadTriangleAngle(float x1, float y1, float x2, float y2, float x3, float y3)
        {
            ...
            if ((apexlen > orglen) && (apexlen > destlen))
            {
                ...
                maxangle = (orglen + destlen - apexlen) / (2 * Mathf.Sqrt(orglen * destlen));
            }
            else if (orglen > destlen)
            {
                // Find the cosine of the angle at the origin.
                maxangle = (apexlen + destlen - orglen) / (2 * Mathf.Sqrt(apexlen * destlen));
            }
            else
            {
                // Find the cosine of the angle at the destination.
                maxangle = (apexlen + orglen - destlen) / (2 * Mathf.Sqrt(apexlen * orglen));
            }
        ...
        private float MinDistanceToNeighbor(float newlocX, float newlocY, ref Otri searchtri)
        {
            ...
            else
            {
                ...
                if (ahead < 0.0f)
                {
                    ...
                    searchtri.Sym();
                    searchtri.Copy(ref horiz);
                    intersect = _TriangleNetMesh.locator.PreciseLocate(newvertex, ref horiz, false);
                }
                else if (ahead == 0.0f)
                {
