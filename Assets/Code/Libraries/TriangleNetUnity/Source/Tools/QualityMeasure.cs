    ...
    public class QualityMeasure
    {
        ...
        private void Compute()
        {
            ...
            foreach (var tri in _TriangleNetMesh.triangles)
            {
                ...
                lx = a.x - b.x;
                ly = a.y - b.y;
                ab = Mathf.Sqrt(lx * lx + ly * ly);
                lx = b.x - c.x;
                ly = b.y - c.y;
                bc = Mathf.Sqrt(lx * lx + ly * ly);
                lx = c.x - a.x;
                ly = c.y - a.y;
                ca = Mathf.Sqrt(lx * lx + ly * ly);
        ...
        {
            if (_TriangleNetMesh == null) return 0;
            ...
            foreach (var tri in _TriangleNetMesh.triangles)
            {
                for (int j = 0; j < 3; j++)
                {
                    ...
                    for (int k = 0; k < 3; k++)
                    {
                        ...
                        mu = Mathf.Max(mu, gj - gi);
                        ml = Mathf.Max(ml, gi - gj);
                    }
                }
            }
        ...
        class AreaMeasure
        {
            ...
            public float Measure(Point a, Point b, Point c)
            {
                ...
                area_min = Mathf.Min(area_min, area);
                area_max = Mathf.Max(area_max, area);
                area_total += area;
        ...
        class AlphaMeasure
        {
            ...
            public float Measure(float ab, float bc, float ca, float area)
            {
                ...
                {
                    a_angle = 2.0f * Mathf.PI / 3.0f;
                    b_angle = 2.0f * Mathf.PI / 3.0f;
                    c_angle = 2.0f * Mathf.PI / 3.0f;
                }
                else
                {
                    if (ca == 0.0f || ab == 0.0f)
                ...
                alpha = Mathf.Min(alpha, a_angle);
                alpha = Mathf.Min(alpha, b_angle);
                alpha = Mathf.Min(alpha, c_angle);

                // Normalize angle from [0,pi/3] radians into qualities in [0,1].
                alpha = alpha * 3.0f / Mathf.PI;

                alpha_ave += alpha;
                alpha_area += area * alpha;

                alpha_min = Mathf.Min(alpha, alpha_min);
                alpha_max = Mathf.Max(alpha, alpha_max);
        ...
        class Q_Measure
        {
            ...
            public float Measure(float ab, float bc, float ca, float area)
            {
                ...
                q_min = Mathf.Min(q_min, q);
                q_max = Mathf.Max(q_max, q);
