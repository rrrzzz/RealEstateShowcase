...
namespace TriangleNet.Meshing
{
    ...
    public static class Converter
    {
        ...
        /// </summary>
        public static TriangleNetMesh ToMesh(Polygon polygon, IList<ITriangle> triangles)
        {
        ...
        /// </summary>
        private static List<Otri>[] SetNeighbors(TriangleNetMesh triangleNetMesh, ITriangle[] triangles)
        {
            Otri tri = default(Otri);
            ...
            var vertexarray = new List<Otri>[triangleNetMesh.vertices.Count];

            // Each vertex is initially unrepresented.
            for (i = 0; i < triangleNetMesh.vertices.Count; i++)
            {
                Otri tmp = default(Otri);
                tmp.tri = triangleNetMesh.dummytri;
            ...
            foreach (var item in triangleNetMesh.triangles)
            {
                ...
                for (int j = 0; j < 3; j++)
                {
                    ...
                    if ((corner[j] < 0) || (corner[j] >= triangleNetMesh.invertices))
                    {
                ...
                // TODO: VarArea
                if (triangleNetMesh.behavior.VarArea)
                {
                ...
                tri.orient = 0;
                tri.SetOrg(triangleNetMesh.vertices[corner[0]]);
                tri.SetDest(triangleNetMesh.vertices[corner[1]]);
                tri.SetApex(triangleNetMesh.vertices[corner[2]]);
                ...
                for (tri.orient = 0; tri.orient < 3; tri.orient++)
                {
                    ...
                    if (checktri.tri.id != TriangleNetMesh.DUMMY)
                    {
                        ...
                        do
                        {
                            ...
                            checktri = nexttri;
                        } while (checktri.tri.id != TriangleNetMesh.DUMMY);
                    }
                }
        ...
        /// </summary>
        private static void SetSegments(TriangleNetMesh triangleNetMesh, Polygon polygon, List<Otri>[] vertexarray)
        {
            Otri checktri = default(Otri);
            ...
            if (triangleNetMesh.behavior.Poly)
            {
                ...
                foreach (var item in triangleNetMesh.subsegs.Values)
                {
                    ...
                    if ((sorg.id < 0 || sorg.id >= triangleNetMesh.invertices) || (sdest.id < 0 || sdest.id >= triangleNetMesh.invertices))
                    {
                    ...
                    for (subseg.orient = 0; subseg.orient < 2; subseg.orient++)
                    {
                        ...
                        while (notfound && (checktri.tri.id != TriangleNetMesh.DUMMY))
                        {
                            checkdest = checktri.Dest();
                            ...
                            if (tmp == checkdest)
                            {
                                ...
                                if (checkneighbor.tri.id == TriangleNetMesh.DUMMY)
                                {
                                    // The next line doesn't insert a subsegment (because there's
                                    // already one there), but it sets the boundary markers of
                                    // the existing subsegment and its vertices.
                                    triangleNetMesh.InsertSubseg(ref checktri, 1);
                                    hullsize++;
                                }
                                notfound = false;
                            }
            ...
            for (i = 0; i < triangleNetMesh.vertices.Count; i++)
            {
                ...
                while (checktri.tri.id != TriangleNetMesh.DUMMY)
                {
                    ...
                    checktri.SegDissolve(triangleNetMesh.dummysub);
                    checktri.Sym(ref checkneighbor);
                    if (checkneighbor.tri.id == TriangleNetMesh.DUMMY)
                    {
                        triangleNetMesh.InsertSubseg(ref checktri, 1);
            ...
            triangleNetMesh.hullsize = hullsize;
        }
