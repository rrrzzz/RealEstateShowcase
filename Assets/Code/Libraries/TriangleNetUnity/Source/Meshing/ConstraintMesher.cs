...
namespace TriangleNet.Meshing
{
    ...
    internal class ConstraintMesher
    {
        ...
        public ConstraintMesher(TriangleNetMesh triangleNetMesh, Configuration config)
        {
            _TriangleNetMesh = triangleNetMesh;
            predicates = config.Predicates();

            behavior = triangleNetMesh.behavior;
            locator = triangleNetMesh.locator;
        ...
        public void Apply(IPolygon input, ConstraintOptions options)
        {
            ...
            if (behavior.Poly && (_TriangleNetMesh.triangles.Count > 0))
            {
                // Copy holes and regions
                _TriangleNetMesh.holes.AddRange(input.Holes);
                _TriangleNetMesh.regions.AddRange(input.Regions);
        ...
        private void CarveHoles()
        {
            ...
            var dummytri = _TriangleNetMesh.dummytri;

            if (!_TriangleNetMesh.behavior.Convex)
            {
            ...
            if (!_TriangleNetMesh.behavior.NoHoles)
            {
                // Infect each triangle in which a hole lies.
                foreach (var hole in _TriangleNetMesh.holes)
                {
                    // Ignore holes that aren't within the bounds of the mesh.
                    if (_TriangleNetMesh.bounds.Contains(hole))
                    {
                        ...
                        if (predicates.CounterClockwise(searchorg, searchdest, hole) > 0.0)
                        {
                            ...
                            intersect = _TriangleNetMesh.locator.Locate(hole, ref searchtri);
            ...
            if (_TriangleNetMesh.regions.Count > 0)
            {
                int i = 0;

                regionTris = new Triangle[_TriangleNetMesh.regions.Count];

                // Find the starting triangle for each region.
                foreach (var region in _TriangleNetMesh.regions)
                {
                    regionTris[i] = dummytri;
                    // Ignore region points that aren't within the bounds of the mesh.
                    if (_TriangleNetMesh.bounds.Contains(region.point))
                    {
                        // Start searching from some triangle on the outer boundary.
                        searchtri.tri = dummytri;
                        searchtri.orient = 0;
                        ...
                        if (predicates.CounterClockwise(searchorg, searchdest, region.point) > 0.0)
                        {
                            ...
                            intersect = _TriangleNetMesh.locator.Locate(region.point, ref searchtri);
                            if ((intersect != LocateResult.Outside) && (!searchtri.IsInfected()))
                            {
            ...
            if (regionTris != null)
            {
                var iterator = new RegionIterator(_TriangleNetMesh);

                for (int i = 0; i < regionTris.Length; i++)
                {
                    if (regionTris[i].id != TriangleNetMesh.DUMMY)
                    {
        ...
        private void FormSkeleton(IPolygon input)
        {
            ...
            _TriangleNetMesh.insegments = 0;

            if (behavior.Poly)
            {
                // If the input vertices are collinear, there is no triangulation,
                // so don't try to insert segments.
                if (_TriangleNetMesh.triangles.Count == 0)
                {
                    return;
                }
        ...
        private void InfectHull()
        {
            ...
            do
            {
                ...
                if (!hulltri.IsInfected())
                {
                    ...
                    if (hullsubseg.seg.hash == TriangleNetMesh.DUMMY)
                    {
                ...
                while (nexttri.tri.id != TriangleNetMesh.DUMMY)
                {
        ...
        void Plague()
        {
            ...
            var dummysub = _TriangleNetMesh.dummysub;
            var dummytri = _TriangleNetMesh.dummytri;
            ...
            for (int i = 0; i < viri.Count; i++)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    {
                        if (neighborsubseg.seg.hash != TriangleNetMesh.DUMMY)
                        {
                            // There is a subsegment separating the triangle from its
                            // neighbor, but both triangles are dying, so the subsegment
                            // dies too.
                            _TriangleNetMesh.SubsegDealloc(neighborsubseg.seg);
            ...
            foreach (var virus in viri)
            {
                ...
                for (testtri.orient = 0; testtri.orient < 3; testtri.orient++)
                {
                    ...
                    if (testvertex != null)
                    {
                        ...
                        while ((neighbor.tri.id != TriangleNetMesh.DUMMY) &&
                ...
                _TriangleNetMesh.TriangleDealloc(testtri.tri);
            }
        ...
        private FindDirectionResult FindDirection(ref Otri searchtri, Vertex searchpoint)
        {
            ...
            if (leftflag && rightflag)
            {
                ...
                if (checktri.tri.id == TriangleNetMesh.DUMMY)
                {
                    leftflag = false;
                }
                else
        ...
        private void SegmentIntersection(ref Otri splittri, ref Osub splitsubseg, Vertex endpoint2)
        {
            ...
            newvertex.hash = _TriangleNetMesh.hash_vtx++;
            ...
            success = _TriangleNetMesh.InsertVertex(newvertex, ref splittri, ref splitsubseg, false, false);
            ...
            if (_TriangleNetMesh.steinerleft > 0)
            {
                _TriangleNetMesh.steinerleft--;
            }
            ...
            do
            {
                ...
                splitsubseg.Next();
            } while (splitsubseg.seg.hash != TriangleNetMesh.DUMMY);
            do
            {
                opposubseg.SetSegOrg(newvertex);
                opposubseg.Next();
            } while (opposubseg.seg.hash != TriangleNetMesh.DUMMY);
        ...
        private bool ScoutSegment(ref Otri searchtri, Vertex endpoint2, int newmark)
        {
                ...
                ((rightvertex.x == endpoint2.x) && (rightvertex.y == endpoint2.y)))
            {
                ...
                _TriangleNetMesh.InsertSubseg(ref searchtri, newmark);
                return true;
            }
            ...
            else
            {
                ...
                if (crosssubseg.seg.hash == TriangleNetMesh.DUMMY)
                {
                    return false;
                }
                else
        ...
        private void DelaunayFixup(ref Otri fixuptri, bool leftside)
        {
            ...
            if (fartri.tri.id == TriangleNetMesh.DUMMY)
            {
                return;
            }
            neartri.Pivot(ref faredge);
            if (faredge.seg.hash != TriangleNetMesh.DUMMY)
            {
                return;
            }
            ...
            _TriangleNetMesh.Flip(ref neartri);
        ...
        private void ConstrainedEdge(ref Otri starttri, Vertex endpoint2, int newmark)
        {
            ...
            _TriangleNetMesh.Flip(ref fixuptri);
            ...
            do
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (crosssubseg.seg.hash == TriangleNetMesh.DUMMY)
                        {
                            _TriangleNetMesh.Flip(ref fixuptri);    // May create inverted triangle at left.
                        }
                        else
                        {
            ...
            _TriangleNetMesh.InsertSubseg(ref fixuptri, newmark);
        ...
        private void MarkHull()
        {
            ...
            // Find a triangle handle on the hull.
            hulltri.tri = _TriangleNetMesh.dummytri;
            hulltri.orient = 0;
            hulltri.Sym();
            // Remember where we started so we know when to stop.
            hulltri.Copy(ref starttri);
            ...
            do
            {
                ...
                _TriangleNetMesh.InsertSubseg(ref hulltri, 1);
                // To find the next hull edge, go clockwise around the next vertex.
                hulltri.Lnext();
                hulltri.Oprev(ref nexttri);
                while (nexttri.tri.id != TriangleNetMesh.DUMMY)
